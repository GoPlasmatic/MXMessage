...
fn test_create_and_validate_pacs008_message() {
    ...
    match document.validate() {
        ...
        Err(e) => {
            ...
            panic!("Message should be valid, but got error: {} (code: {})", e.message, e.code);
        }
    }
}
...
fn test_json_serialization_roundtrip() {
    ...
    // Serialize to JSON
    let json_str = serde_json::to_string(&original_document)
        .expect("Should serialize to JSON");
    
    // Deserialize from JSON
    let deserialized_document: Document = serde_json::from_str(&json_str)
        .expect("Should deserialize from JSON");
    
    // Validate deserialized message
    assert!(deserialized_document.validate().is_ok(), "Deserialized message should be valid");
    
    // Compare original and deserialized
    assert_eq!(original_document, deserialized_document, "Original and deserialized should be equal");
}
...
fn test_validation_errors() {
    ...
    if let Document::FIToFICustomerCreditTransferV13(ref mut msg) = document {
        msg.grp_hdr.msg_id = "".to_string(); // Invalid: empty message ID
    }
    
    let validation_result = document.validate();
    assert!(validation_result.is_err(), "Should fail validation with empty message ID");
...
fn test_validation_iban_format() {
    ...
    if let Document::FIToFICustomerCreditTransferV13(ref mut msg) = document {
        ...
        if let Some(ref mut debtor_account) = msg.cdt_trf_tx_inf[0].dbtr_acct {
            if let Some(ref mut account_id) = debtor_account.id {
                account_id.iban = Some("INVALID_IBAN".to_string());
            }
        }
    }
    
    let validation_result = document.validate();
    assert!(validation_result.is_err(), "Should fail validation with invalid IBAN");
...
fn test_message_id_length_validation() {
    ...
    let validation_result = document.validate();
    assert!(validation_result.is_err(), "Should fail validation with too long message ID");
...
fn test_number_of_transactions_pattern() {
    ...
    let validation_result = document.validate();
    assert!(validation_result.is_err(), "Should fail validation with non-numeric nb_of_txs");
...
fn test_unknown_document_type() {
    ...
    let validation_result = document.validate();
    assert!(validation_result.is_err(), "Unknown document type should fail validation");
...
fn test_minimal_valid_message() {
    ...
    assert!(document.validate().is_ok(), "Minimal message should be valid");
    
    // Should serialize/deserialize successfully
    let json_str = serde_json::to_string(&document)
        .expect("Should serialize minimal message");
    
    let deserialized: Document = serde_json::from_str(&json_str)
        .expect("Should deserialize minimal message");
    
    assert_eq!(document, deserialized, "Minimal message roundtrip should work");
}
