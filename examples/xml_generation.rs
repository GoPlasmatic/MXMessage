...
fn demonstrate_pacs008_scenarios() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let standard_xml = generate_sample_xml("pacs008", Some("standard"), &ScenarioConfig::default())
...
fn demonstrate_different_message_types() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pain001_xml = generate_sample_xml("pain001", Some("standard"), &ScenarioConfig::default())
        .map_err(|e| format!("Failed to generate sample: {e:?}"))?;
    ...
    let pacs009_xml = generate_sample_xml("pacs009", Some("standard"), &ScenarioConfig::default())
        .map_err(|e| format!("Failed to generate sample: {e:?}"))?;
...
fn demonstrate_xml_configurations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let camt056_xml = generate_sample_xml(
        "camt056",
        Some("cbpr_cancellation_request"),
        &ScenarioConfig::default(),
    )
    .map_err(|e| format!("Failed to generate sample: {e:?}"))?;
    ...
    let pacs002_xml = generate_sample_xml("pacs002", None, &ScenarioConfig::default())
        .map_err(|e| format!("Failed to generate sample: {e:?}"))?;
