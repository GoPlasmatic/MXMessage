    ...
    pub fn validate(&self) -> Result<(), ValidationError> {
        ...
        if let Some(ref val) = self.orgnl_msg_dlvry_chanl {
            if val.chars().count() < 1 {
                return Err(ValidationError::new(
                    1001,
                    "orgnl_msg_dlvry_chanl is shorter than the minimum length of 1".to_string(),
                ));
            }
            if val.chars().count() > 35 {
                return Err(ValidationError::new(
                    1002,
                    "orgnl_msg_dlvry_chanl exceeds the maximum length of 35".to_string(),
                ));
            }
        }
        Ok(())
...
pub struct UnderlyingStatementEntry3 {
    ...
    #[serde(rename = "OrgnlNtryId", skip_serializing_if = "Option::is_none")]
    ...
    pub fn validate(&self) -> Result<(), ValidationError> {
        ...
        if let Some(ref val) = self.orgnl_ntry_id {
            if val.chars().count() < 1 {
                return Err(ValidationError::new(
                    1001,
                    "orgnl_ntry_id is shorter than the minimum length of 1".to_string(),
                ));
            }
            if val.chars().count() > 35 {
                return Err(ValidationError::new(
                    1002,
                    "orgnl_ntry_id exceeds the maximum length of 35".to_string(),
                ));
            }
        }
        if let Some(ref val) = self.orgnl_uetr {
...
impl UnderlyingTransaction5Choice {
    pub fn validate(&self) -> Result<(), ValidationError> {
        ...
        if let Some(ref val) = self.stmt_ntry {
            val.validate()?
        }
        Ok(())
    }
