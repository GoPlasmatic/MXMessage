...
impl AccountIdentification4Choice2 {
    pub fn validate(&self) -> Result<(), ValidationError> {
        if let Some(ref val) = self.iban {
            let pattern = Regex::new("[A-Z]{2,2}[0-9]{2,2}[a-zA-Z0-9]{1,30}").unwrap();
            if !pattern.is_match(val) {
    ...
    pub fn validate(&self) -> Result<(), ValidationError> {
        if let Some(ref val) = self.orgnl_mndt_id {
            ...
            if !pattern.is_match(val) {
                return Err(ValidationError::new(
                    1005,
                    "orgnl_mndt_id does not match the required pattern".to_string(),
                ));
            }
        }
    ...
    pub fn validate(&self) -> Result<(), ValidationError> {
        ...
        if let Some(ref val) = self.pre_ntfctn_id {
            ...
            if !pattern.is_match(val) {
                return Err(ValidationError::new(
                    1005,
                    "pre_ntfctn_id does not match the required pattern".to_string(),
                ));
            }
        }
        Ok(())
...
impl DirectDebitTransactionInformation241 {
    pub fn validate(&self) -> Result<(), ValidationError> {
...
impl FIToFICustomerDirectDebitV08 {
    pub fn validate(&self) -> Result<(), ValidationError> {
...
impl GenericAccountIdentification12 {
    pub fn validate(&self) -> Result<(), ValidationError> {
        if self.id.chars().count() < 1 {
            return Err(ValidationError::new(
                1001,
...
impl MandateSetupReason1Choice1 {
    pub fn validate(&self) -> Result<(), ValidationError> {
        if let Some(ref val) = self.cd {
            if val.chars().count() < 1 {
                return Err(ValidationError::new(
    ...
    pub fn validate(&self) -> Result<(), ValidationError> {
        if let Some(ref val) = self.dept {
            ...
            if val.chars().count() > 35 {
                return Err(ValidationError::new(
                    1002,
                    "dept exceeds the maximum length of 35".to_string(),
                ));
            }
            let pattern = Regex::new(
                "[0-9a-zA-Z/\\-\\?:\\(\\)\\.,'\\+ !#$%&\\*=^_`\\{\\|\\}~\";<>@\\[\\\\\\]]+",
        ...
        if let Some(ref val) = self.sub_dept {
            ...
            if val.chars().count() > 35 {
                return Err(ValidationError::new(
                    1002,
                    "sub_dept exceeds the maximum length of 35".to_string(),
                ));
            }
            let pattern = Regex::new(
                "[0-9a-zA-Z/\\-\\?:\\(\\)\\.,'\\+ !#$%&\\*=^_`\\{\\|\\}~\";<>@\\[\\\\\\]]+",
        ...
        if let Some(ref val) = self.strt_nm {
            ...
            if val.chars().count() > 35 {
                return Err(ValidationError::new(
                    1002,
                    "strt_nm exceeds the maximum length of 35".to_string(),
                ));
            }
            let pattern = Regex::new(
                "[0-9a-zA-Z/\\-\\?:\\(\\)\\.,'\\+ !#$%&\\*=^_`\\{\\|\\}~\";<>@\\[\\\\\\]]+",
        ...
        if let Some(ref val) = self.flr {
            ...
            if val.chars().count() > 35 {
                return Err(ValidationError::new(
                    1002,
                    "flr exceeds the maximum length of 35".to_string(),
                ));
            }
            let pattern = Regex::new(
                "[0-9a-zA-Z/\\-\\?:\\(\\)\\.,'\\+ !#$%&\\*=^_`\\{\\|\\}~\";<>@\\[\\\\\\]]+",
        ...
        if let Some(ref val) = self.room {
            ...
            if val.chars().count() > 35 {
                return Err(ValidationError::new(
                    1002,
                    "room exceeds the maximum length of 35".to_string(),
                ));
            }
            let pattern = Regex::new(
                "[0-9a-zA-Z/\\-\\?:\\(\\)\\.,'\\+ !#$%&\\*=^_`\\{\\|\\}~\";<>@\\[\\\\\\]]+",
