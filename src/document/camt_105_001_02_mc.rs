...
pub struct ChargesPerTransaction41 {
    ...
    pub chrgs_id: String,
    #[serde(rename = "Rcrd")]
    pub rcrd: Vec<ChargesPerTransactionRecord41>,
}

impl ChargesPerTransaction41 {
    pub fn validate(&self) -> Result<(), ValidationError> {
...
impl ChargesPerTransactionRecord41 {
    pub fn validate(&self) -> Result<(), ValidationError> {
        if let Some(ref val) = self.rcrd_id {
            if val.chars().count() < 1 {
                return Err(ValidationError::new(
                    1001,
                    "rcrd_id is shorter than the minimum length of 1".to_string(),
                ));
            }
            if val.chars().count() > 35 {
                return Err(ValidationError::new(
                    1002,
                    "rcrd_id exceeds the maximum length of 35".to_string(),
                ));
            }
            let pattern = Regex::new("[0-9a-zA-Z/\\-\\?:\\(\\)\\.,'\\+ ]+").unwrap();
            if !pattern.is_match(val) {
                return Err(ValidationError::new(
                    1005,
                    "rcrd_id does not match the required pattern".to_string(),
                ));
            }
        }
...
impl GroupHeader1261 {
    pub fn validate(&self) -> Result<(), ValidationError> {
        ...
        self.ttl_chrgs.validate()?;
        self.chrgs_acct.validate()?;
...
impl TotalCharges71 {
    pub fn validate(&self) -> Result<(), ValidationError> {
        ...
        if !pattern.is_match(&self.nb_of_chrgs_rcrds) {
            return Err(ValidationError::new(
                1005,
                "nb_of_chrgs_rcrds does not match the required pattern".to_string(),
            ));
        }
