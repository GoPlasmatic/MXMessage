...
// Cheque131: Party that receives an amount of money as specified in the cheque.
#[derive(Debug, Default, Serialize, Deserialize, Clone, PartialEq)]
pub struct Cheque131 {
    ...
    #[serde(rename = "Pyer")]
    pub pyer: PartyIdentification1351,
    #[serde(rename = "PyerAcct", skip_serializing_if = "Option::is_none")]
    pub pyer_acct: Option<CashAccount401>,
    ...
    #[serde(rename = "Pyee")]
    pub pyee: PartyIdentification1352,
}
...
impl Validate for Cheque131 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        ...
        self.pyer
            .validate(&helpers::child_path(path, "Pyer"), config, collector);
        if let Some(ref val) = self.pyer_acct {
            if config.validate_optional_fields {
                val.validate(&helpers::child_path(path, "PyerAcct"), config, collector);
            }
        }
        ...
        self.pyee
            .validate(&helpers::child_path(path, "Pyee"), config, collector);
    }
}

// ChequePresentmentNotificationV01: Specifies the details of the cheque.
#[derive(Debug, Default, Serialize, Deserialize, Clone, PartialEq)]
