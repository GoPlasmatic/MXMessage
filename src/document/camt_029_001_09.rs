...
impl Validate for InvestigationStatus5Choice1 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        if let Some(ref val) = self.conf
            && config.validate_optional_fields
        {
            val.validate(&helpers::child_path(path, "Conf"), config, collector);
        }
    }
}
...
impl Validate for PaymentTransaction1021 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        helpers::validate_length(
            ...
            Some(1),
            Some(16),
            &helpers::child_path(path, "CxlStsId"),
            config,
            collector,
        );
        helpers::validate_pattern(
        ...
        self.rslvd_case
            .validate(&helpers::child_path(path, "RslvdCase"), config, collector);
        self.orgnl_grp_inf
        ...
        {
            val.validate(
                &helpers::child_path(path, "CxlStsRsnInf"),
                config,
                collector,
            );
        }
...
impl Validate for ResolutionOfInvestigationV09 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        ...
        self.cxl_dtls
            .validate(&helpers::child_path(path, "CxlDtls"), config, collector);
    }
}
