...
// CBPR_CancellationReasonCode: Reason is provided as narrative information in the additional reason information.
#[derive(Debug, Default, Serialize, Deserialize, Clone, PartialEq)]
pub enum CBPRCancellationReasonCode {
    #[default]
    #[serde(rename = "DUPL")]
    CodeDUPL,
    #[serde(rename = "CUTA")]
...
pub struct CancellationReason33Choice1 {
    ...
    pub cd: Option<CBPRCancellationReasonCode>,
}

impl Validate for CancellationReason33Choice1 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
...
impl Validate for Case51 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        ...
        self.cretr
            .validate(&helpers::child_path(path, "Cretr"), config, collector);
    }
}
...
impl Validate for CaseAssignment51 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        helpers::validate_length(
            &self.id,
            "Id",
        ...
        self.assgnr
            .validate(&helpers::child_path(path, "Assgnr"), config, collector);
        self.assgne
            .validate(&helpers::child_path(path, "Assgne"), config, collector);
        helpers::validate_pattern(
            &self.cre_dt_tm,
            "CreDtTm",
            ".*(\\+|-)((0[0-9])|(1[0-4])):[0-5][0-9]",
...
// FIToFIPaymentCancellationRequestV08: Identifies the payment instruction to be cancelled.
#[derive(Debug, Default, Serialize, Deserialize, Clone, PartialEq)]
pub struct FIToFIPaymentCancellationRequestV08 {
    #[serde(rename = "Assgnmt")]
    pub assgnmt: CaseAssignment51,
    #[serde(rename = "Undrlyg")]
    pub undrlyg: UnderlyingTransaction231,
}

impl Validate for FIToFIPaymentCancellationRequestV08 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        self.assgnmt
            .validate(&helpers::child_path(path, "Assgnmt"), config, collector);
        self.undrlyg
            .validate(&helpers::child_path(path, "Undrlyg"), config, collector);
    }
}
...
impl Validate for PaymentCancellationReason51 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
...
// PaymentTransaction1061: Provides detailed information on the cancellation reason.
#[derive(Debug, Default, Serialize, Deserialize, Clone, PartialEq)]
pub struct PaymentTransaction1061 {
    ...
    pub orgnl_intr_bk_sttlm_amt: CBPRAmount,
...
impl Validate for PaymentTransaction1061 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        ...
        self.case
            .validate(&helpers::child_path(path, "Case"), config, collector);
        self.orgnl_grp_inf
        ...
        self.cxl_rsn_inf
            .validate(&helpers::child_path(path, "CxlRsnInf"), config, collector);
    }
}
...
// UnderlyingTransaction231: Provides information on the original transactions to which the cancellation request message refers.
#[derive(Debug, Default, Serialize, Deserialize, Clone, PartialEq)]
pub struct UnderlyingTransaction231 {
    #[serde(rename = "TxInf")]
    pub tx_inf: PaymentTransaction1061,
}
