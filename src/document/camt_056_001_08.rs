...
impl CBPRCancellationReasonCode {
    pub fn validate(&self) -> Result<(), ValidationError> {
        Ok(())
    }
}
    ...
    pub fn validate(&self) -> Result<(), ValidationError> {
        if let Some(ref val) = self.cxl_id {
            ...
            if val.chars().count() > 16 {
                return Err(ValidationError::new(
                    1002,
                    "cxl_id exceeds the maximum length of 16".to_string(),
                ));
            }
            let pattern = Regex::new("[0-9a-zA-Z/\\-\\?:\\(\\)\\.,'\\+ ]+").unwrap();
            if !pattern.is_match(val) {
                return Err(ValidationError::new(
                    1005,
                    "cxl_id does not match the required pattern".to_string(),
                ));
            }
        }
        self.case.validate()?;
