...
impl Validate for Charges3Choice1 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
...
impl Validate for ChargesPaymentRequestV02 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        self.grp_hdr
...
pub struct ChargesPerTransaction31 {
    ...
    pub chrgs_id: String,
    #[serde(rename = "Rcrd")]
    pub rcrd: Vec<ChargesPerTransactionRecord31>,
}

impl Validate for ChargesPerTransaction31 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        helpers::validate_length(
            &self.chrgs_id,
            "ChrgsId",
...
impl Validate for ChargesPerTransactionRecord31 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        if let Some(ref val) = self.rcrd_id {
            helpers::validate_length(
                ...
                Some(1),
                Some(35),
                &helpers::child_path(path, "RcrdId"),
                config,
                collector,
            );
        }
        ...
        {
            val.validate(
                &helpers::child_path(path, "InstrForInstdAgt"),
                config,
                collector,
            );
        }
...
impl Validate for GroupHeader1151 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        helpers::validate_length(
            &self.msg_id,
            "MsgId",
        ...
        self.ttl_chrgs
            .validate(&helpers::child_path(path, "TtlChrgs"), config, collector);
        ...
        {
            val.validate(
                &helpers::child_path(path, "ChrgsAcctAgtAcct"),
                config,
                collector,
            );
        }
...
impl Validate for InstructionForInstructedAgent11 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        helpers::validate_length(
            &self.cd,
            "Cd",
...
// TotalCharges71: Indicates whether the total charges amount is a credit or a debit amount.
// Usage: A zero amount is considered to be a credit.
#[derive(Debug, Default, Serialize, Deserialize, Clone, PartialEq)]
pub struct TotalCharges71 {
...
impl Validate for TotalCharges71 {
    fn validate(&self, path: &str, config: &ParserConfig, collector: &mut ErrorCollector) {
        helpers::validate_pattern(
            &self.nb_of_chrgs_rcrds,
            "NbOfChrgsRcrds",
            "[0-9]{1,15}",
            &helpers::child_path(path, "NbOfChrgsRcrds"),
            config,
            collector,
        );
        self.ttl_chrgs_amt
